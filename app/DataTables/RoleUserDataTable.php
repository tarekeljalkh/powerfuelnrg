        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                if ($query->getRoleNames()->first() !== 'Super Admin') {
                    $edit = '<a href="' . route('role-user.edit', $query->id) . '" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></a>';
                    $delete = '<a href="' . route('role-user.destroy', $query->id) . '" class="delete-item btn btn-sm btn-danger ml-2"><i class="fas fa-trash"></i></a>';
